...
ApexHUD 0.11.0 использует schema version `1`.
...
ApexHUD 0.11.0 не требует ready-handshake и сейчас не выполняет действие по этому сообщению. Его можно отправлять для будущей совместимости, но инициализация модуля не должна ждать подтверждения.
...
`status` — диагностический текст, его нельзя парсить как стабильный enum. Auto-hide уже применяет host, поэтому большинству модулей `simulatorWindow` не нужен.
...
Unlimited sentinels iRacing нормализуются: отсутствующий лимит равен `0`, а соответствующий `has*Limit` — false. `flags` является bitfield; используйте bitwise checks. Неизвестные `state`, flags и session strings должны обрабатываться безопасно.
...
`carIndex` — самый надёжный live identity. Position `0` и lap time `0` означают unavailable. В мультиклассе overall и class position подписываются явно.
...
Processed `throttle`/`brake` учитывают in-sim shaping и cuts; `*Raw` показывает положение педалей, если доступно. Controls и shift percentages нормализованы в `0..1`. Для progressive lights предпочтителен `shiftIndicatorPercent`, для фактического limiter state — `driverAids.revLimiterActive`; RPM thresholds остаются fallback. Не заставляйте передачу мигать задолго до порога симулятора.
...
`absActive` — фактическое вмешательство ABS. `absCutPercent` — reported brake-cut fraction. TC fields описывают доступность, enabled state и levels; универсальный `tcActive` намеренно не публикуется, потому что SDK не даёт надёжный cross-car intervention signal. При false availability не показывайте paired false/zero как подтверждённое состояние.
...
Service-selection fields описывают выбранное обслуживание, а не факт завершения работ. Status/flags/compound считаются открытыми numeric values без документированного mapping.
...
До полной инициализации сессии weather fields могут отсутствовать. `trackWetness` и `skies` — source enums; принимайте неизвестные значения. `weatherDeclaredWet` означает разрешение wet tires race control, а не просто наличие дождя.
...
Motion подходит для training, g-force и vehicle-attitude modules. Данные могут шуметь: фильтруйте визуал, но не создавайте заметную задержку.
...
Отрицательная delta означает быстрее. У каждой delta свой availability flag; не показывайте `0.000` как реальное сравнение при unavailable.
...
До `estimateReady` показывайте спокойный learning state. Estimates являются локальной подсказкой, а не official scoring.
...
Radar — улучшенная spotter-аппроксимация, а не world-space geometry. `overlap` — нормализованная глубина нахождения бок о бок, не физическое пересечение. Отрицательный `carIndex` может обозначать synthetic contact по авторитетному side state iRacing.
...
Relative — данные spatial awareness. `estimatedGapSeconds` рассчитан по wrapped progress и текущей скорости и не является official scoring interval.
...
Race positions стабилизируются против one-frame jumps на timing line. Для grouping используйте `carClassId`, для display — `carClassName`. Игрок определяется по `isPlayer` с fallback `carIndex === player.carIndex` и должен оставаться видимым вне top N.
...
В ApexHUD 0.11.0 schema содержит по одной строке `name`, `description`, `label` и `help`. Host пока не поддерживает localized maps и не отправляет выбранный язык Control Center в `apex:init`.
...
ApexHUD 0.11.0 отправляет protocol version `6` в `apex:init` и `apex:frame`.
...
На ApexHUD 0.11.0 публичный API модулей не предоставляет:
...
Для точной реализации ApexHUD 0.11.0 смотрите:
