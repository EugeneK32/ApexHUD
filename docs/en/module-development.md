...
ApexHUD 0.11.0 uses manifest schema version `1`.
...
ApexHUD 0.11.0 does not require a ready handshake and currently performs no runtime action for this message. It is safe to emit for forward compatibility, but module initialization must not depend on an acknowledgement.
...
`status` is diagnostic text and must not be parsed as a stable enum. The host already applies the selected auto-hide policy, so most modules do not need `simulatorWindow`.
...
Unlimited iRacing sentinels are normalized: absent limits use `0` and the matching `has*Limit` flag is false. `flags` is a bitfield; use bitwise checks rather than numeric equality. `state`, flags, and future session strings should be treated defensively.
...
Use `carIndex` as the strongest live identity. Position `0` and lap time `0` mean unavailable. In multiclass widgets label overall and class position explicitly.
...
Processed `throttle`/`brake` include in-sim shaping and cuts; `*Raw` values represent pedal-side input when the simulator exposes them. All controls and shift percentages are normalized to `0..1`. Prefer `shiftIndicatorPercent` for progressive lights and `driverAids.revLimiterActive` for actual limiter state; use the car-specific RPM thresholds as fallback. Do not flash a gear indicator far below the simulator threshold.
...
`absActive` is actual ABS intervention. `absCutPercent` is the reported brake-cut fraction when available. TC fields describe availability, enabled state, and configured levels; ApexHUD deliberately does **not** publish a generic `tcActive`, because the SDK does not provide a reliable cross-car intervention signal. When an availability flag is false, do not render the paired false/zero value as confirmed state.
...
The service-selection fields describe the currently configured pit service, not a guarantee that work has completed. Treat status/flags/compound as open numeric values unless your module maintains a documented mapping.
...
Weather values may remain absent before a session is fully initialized. `trackWetness` and `skies` are source enums; accept unknown future numbers. `weatherDeclaredWet` indicates that race control permits wet tires, not merely that rain exists.
...
Motion is suitable for training, g-force, and vehicle-attitude widgets. Values can be noisy; filter for display, but do not add enough smoothing to create visible latency.
...
Negative deltas mean ahead/faster. Every delta has its own availability flag; do not display `0.000` as a real comparison when unavailable.
...
Use a calm learning state until `estimateReady` is true. Estimates are local strategy aids, not official scoring values.
...
Radar is an enhanced spotter approximation, not exact world-space geometry. `overlap` is normalized side-by-side depth, not physical intersection. Negative `carIndex` may represent a synthetic contact required by the authoritative iRacing side state.

### 11.12 `relative`

```ts
interface RelativeState { entries: RelativeEntry[]; }
...
Relative entries are spatial awareness data. `estimatedGapSeconds` is based on wrapped progress and current speed; it is not an official scoring interval.
...
Race positions are stabilized to suppress one-frame timing-line jumps. Use `carClassId` for grouping and `carClassName` for display. Always identify the player by `isPlayer` with `carIndex === player.carIndex` as fallback, and keep the player visible outside top N.

### Detecting multiclass

```js
function classCount(entries) {
  return new Set(entries.map((entry) => entry.carClassId)).size;
}
```
...
ApexHUD 0.11.0 manifest schema has one `name`, `description`, `label` and `help` string. The host does not currently support localized manifest string maps and does not send the selected Control Center locale in `apex:init`.
...
ApexHUD 0.11.0 sends protocol version `6` in `apex:init` and `apex:frame`.
...
```text
Requires ApexHUD 0.11.0 or later (telemetry protocol v6).
```
...
As of ApexHUD 0.11.0, public module API does not provide:
...
For the exact implementation in ApexHUD 0.11.0, consult:
