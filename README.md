...
ApexHUD is licensed under **MPL-2.0**. The current telemetry service depends on GPL-licensed IRSDKSharper, so publishing the combined project under a weaker incompatible license would be incorrect. See [LICENSE](LICENSE), [Licensing](docs/en/licensing.md), and [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).

The ApexHUD name, logo, and visual identity are handled separately; see [TRADEMARKS.md](TRADEMARKS.md).
